...
(
    ...
    envsubst '${OWGE_BACKEND_SERVER} ${OWGE_FRONTEND_SERVER} ${OWGE_ACCOUNT_SERVER} ${OWGE_NGINX_PHPMYADMIN_SERVER} ${OWGE_REST_CONTEXT_PATH} ${OWGE_ADMIN_CONTEXT_PATH}' < /etc/nginx/conf.d/proxy_settings.template > /etc/nginx/conf.d/proxy_settings.conf
)
exec "$@"