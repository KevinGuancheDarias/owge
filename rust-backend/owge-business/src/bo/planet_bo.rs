...
impl PlanetBo {
    ...
    pub async fn find_owned_dtos(db: &Db, user_id: i32) -> OwgeResult<Vec<PlanetDto>> {
    ...
    pub async fn can_leave_planet(db: &Db, user_id: i32, planet_id: u64) -> OwgeResult<bool> {
        ...
        if MissionFinderBo::has_running_unit_build(db, user_id, planet_id).await? {
            return Ok(false);
        }
    ...
    pub async fn leave_planet(db: &Db, user_id: i32, planet_id: u64) -> OwgeResult<()> {
        if !Self::can_leave_planet(db, user_id, planet_id).await? {
        ...
        // trigger's writes (this method runs on the db pool, no surrounding tx, so
        // we wrap the trigger in its own transaction for a consistent view).
        ...
        for holder_id in list_holders {
            realtime_emitter::emit_planet_user_list_change(db, holder_id).await?;
        }
        ...
        realtime_emitter::emit_planet_owned_change(db, user_id).await?;
        ...
        realtime_emitter::drain_requirement_emits(db, &req_emits).await?;
