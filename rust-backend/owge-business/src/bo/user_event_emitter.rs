...
//!
//! Both follow the canonical wrapper shape: async fn (conn, user_id) calling
...
impl UserEventEmitter {
    ...
    pub async fn emit_max_energy_change(
        conn: &mut MySqlConnection,
        user_id: i32,
    ) -> OwgeResult<()> {
        emitter::send_message(conn, user_id, "user_max_energy_change", |conn| {
            Box::pin(async move {
                let max_energy = Self::find_max_energy(&mut *conn, user_id).await?;
    ...
    pub async fn find_max_energy(conn: &mut MySqlConnection, user_id: i32) -> OwgeResult<f64> {
