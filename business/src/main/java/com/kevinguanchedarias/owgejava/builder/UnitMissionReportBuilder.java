...
public class UnitMissionReportBuilder {
    ...
    public UnitMissionReportBuilder withUnitCaptureInformation(List<UnitCaptureContext> unitCaptureContext) {
        List<Map<String, Object>> unitCaptureInformation = new ArrayList<>();
        unitCaptureContext.forEach(current -> {
            var victim = current.getVictimUnit();
            Map<String, Object> entry = new HashMap<>();
            entry.put("unit", obtainedUnitToDto(victim.getObtainedUnit()).getUnit());
            entry.put("oldOwner", userToDto(victim.getUser().getUser()));
            entry.put("capturedCount", current.getCapturedUnits());
            unitCaptureInformation.add(entry);
        });
        createdMap.put("unitCaptureInformation", unitCaptureInformation);
        return this;
    }
