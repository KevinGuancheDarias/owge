...
* __Fix:__ `RustBackend:` units restricted to owned planets (OWNED_ONLY mission support) could be sent to any planet — the restriction is now enforced like the Java backend; rejected mission registrations also no longer consume internal ids
* __Fix:__ `RustBackend:` units returning to a planet that changed owner mid-flight were stranded without a mission; they now park as a deployed stack on the (now foreign) planet, like the Java backend
* __Fix:__ `RustBackend:` when a mission crashes and is retried/given up, the client now receives the refreshed mission state (running upgrade, build list, or unit/enemy missions) like the Java backend sends
* __Fix:__ `RustBackend:` special-location planets showed the placeholder icon instead of their image in the owned-planets list, mission source/target planets, and mission reports — the payloads dropped the location's image/imageUrl (and improvement/galaxy info); every planet payload now carries the full special location like the Java backend
* __Fix:__ `RustBackend:` mission executions no longer risk deadlocking on the planet locks: GET_LOCK is now issued strictly one key per statement in ascending order, stopping at the first unavailable key and releasing the acquired prefix before backing off (the same v0.11.14 fix the Java backend received — the old multi-GET_LOCK statement kept acquiring later keys after an early one timed out, breaking the global lock order); lock wait timeout lowered 10s→3s with 200ms-per-attempt retry backoff
* __Fix:__ `RustBackend:` a crashed mission's retry is no longer lost (the scheduler cleared the retry it had just written — same class as the Java v0.11.14 lost-retry fix; the retry instant now re-arms the task row), and the "mission failed, contact an admin" report is only sent when the mission is definitively given up after all attempts, not on every retry
* __Improvement:__ new admin action "Repair special-location unlocks" (admin panel index + `POST admin/special-location/repair-unlocks` on both backends): re-grants the HAVE_SPECIAL_LOCATION unlocks to every current owner of a special-location planet — run once per universe to heal players affected by the old unlocks-never-granted bug (idempotent, safe to re-run)
...
* __Fix:__ `RustBackend:` fixed bad json signature for user_data_change sync event
...
* __Fix:__ [Sync system doesn't properly handle browser disconection and reconnection, events occurring offline are not sent to the browser when online again #527](https://github.com/KevinGuancheDarias/owge/issues/527)
* __Fix:__ [When cancelling a mission, if it's not in the scheduler, it's failing #544](https://github.com/KevinGuancheDarias/owge/issues/544)  
